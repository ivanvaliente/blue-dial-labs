(() => {
  ...
  const setPerspective = (perspective, updateHash = true) => {
    ...
    if (updateHash) {
      history.replaceState(null, '', isPartner ? '#partner' : '#collector');
    }
  };
  ...
  const bindPreferenceControl = () => {
    document.querySelectorAll('input[name="demo-preference"]').forEach((input) => {
    ...
    document.querySelectorAll('[data-demo-action]').forEach((button) => {
      button.addEventListener('click', () => {
        ...
        if (button.dataset.demoAction === 'view') {
          status.textContent = `Demo only — View Offer would continue to ${offer.seller} for the seller’s existing checkout. No live offer is opened.`;
        } else if (button.dataset.demoAction === 'wait') {
          status.textContent = 'Wait selected — the Pursuit would remain active while Scout continues watching supported supply.';
        } else {
          status.textContent = 'Reject selected — this synthetic offer would be dismissed without ending the Pursuit.';
        }
      });
    });
  };
